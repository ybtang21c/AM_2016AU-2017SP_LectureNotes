...
\begin{enumerate}[(1)]
  ...
  \item $\dint\df{Q(x)}{(x-a)^n}$，令$u=x-a$\\
  例如： $\dint\df x{(x+1)^3}\d x=\dint\df{(u-1)^3}{u^3}\d u$ 
  \item $\dint\df{Cx+D}{x^2+px+q}\d x=
  \df C2\ln(x^2+px+q)+\df{2D-Cp}{\sqrt{4q-p^2}}
  \arctan\df{2x+p}{\sqrt{4q-p^2}}+C_1$\ps{假设$\Delta=p^2-4q<0$}
...
{\bf 注：}1、关于（4），{\b 形如$\dint\df{\d x}{x^2+px+q}$的积分，
记$\Delta=p^2-4q$，
...
% 	=\df12\dint\df{x\d x^2}{(1+x^2)^2}
% 	=\df12\dint\df{x\d(1+x^2)}{(1+x^2)^2}
% 	=-\df12\dint x\d\df1{1+x^2}\notag\\
% 	&=\df{x}{2(1+x^2)}\notag
% \end{align}
