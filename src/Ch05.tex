...
{\bf 例：}通常情况下，声波上层岩石中的传播速度$c_1$小于在下层岩石中的传播速度$c_2$。
如图所示，求
\begin{enumerate}[(1)]
  \setlength{\itemindent}{1cm}
...
\begin{itemize}
  \setlength{\itemindent}{1cm}
  \item $f(\xi)+\lambda f'(\xi)=0$： $F(x)=e^{\lambda x}f(x)$
  \item $f'(\xi)+f(\xi)g'(\xi)=0$： $F(x)=e^{\lambda g(x)}f(x)$
  \item $nf(\xi)+xf'(\xi)=0$： $F(x)=x^nf(x)$ 
  \item $f'(\xi)g(\xi)+f(\xi)g'(\xi)=0$：$F(x)=f(x)g(x)$
...
$$f(b)-f(a)=f'(\xi)(b-a)$$
$$f(x+\Delta x)=f(x)+f'(x+\theta\Delta x)(b-a),(\theta\in(0,1))$$
...
{\bf P237-例2：}证明不等式：$\df{x}{1+x}<\ln(1+x)<x,\;x>0$
...
参考L'Hospital法则的正面原理，可以证明下面的两个结论：
...
\begin{enumerate}[(1)]
  ...
  \item $\varphi^{(k)}(a)=\psi^{(k)}(a),k=0,1,2,\ldots,n-1$
\end{enumerate}
则在$(a,b)$上，恒有$\varphi(x)>\psi(x)$
...
{\bf 提示：}令$f(x)=x(1-x)$，可求得其最大值为$\df14$。若以上两数都大于$1/4$，则
$$a(1-b)b(1-a)>\df1{16},$$
这与前述最大值的性质矛盾。

{\bf 例：}当$a$取何值时，$a^x\geq1+x$恒成立。

{\bf 提示：}两条曲线总有交点$(0,1)$，又$(a^x)''=a^x\ln^2x>0$，故$y=a^x$上凹。
...
\begin{center}
	\resizebox{!}{3cm}{\includegraphics{./images/ch5/curSphere.pdf}}
\end{center}
